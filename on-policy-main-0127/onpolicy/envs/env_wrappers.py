...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == 'get_avaliable_action':
            mask = env.get_avaliable_action()
            remote.send((mask))
        else:
            raise NotImplementedError
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def get_avaliable_action(self):
        for remote in self.remotes:
            remote.send(('get_avaliable_action',None))

        avaliable_action = [remote.recv() for remote in self.remotes]
        return np.stack(avaliable_action)
