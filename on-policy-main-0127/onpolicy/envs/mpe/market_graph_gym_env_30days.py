...
class MA_ELEC_Market_Graph_ENV(gym.Env):
    ...
    def __init__(self, max_step, custom_agent_num, gate_num=10):
        ...
        # csv_data_l = pd.read_excel(MainPath + '/onpolicy/envs/mpe/30daysData/month_l_118.xlsx') # 0116
        # csv_data_p = pd.read_excel(MainPath + '/onpolicy/envs/mpe/30daysData/month_p_118.xlsx') # 0116

        # print('custom',self.custom_agent_num)
        self.bus_data_selection = pd.read_excel(MainPath + '/onpolicy/envs/mpe/30daysData/118bus_data.xlsx',
                                                sheet_name=None)
        self.calc_30days_params()  # 0119
        self.init_every_day()
    ...
    def calc_30days_params(self):
        # 0119
        E_arrays = []
        L_MaxArray = []
        for ind in range(0, 30):
            csv_data_l = self.bus_data_selection['FL'].iloc[ind * 24:(ind + 1) * 24, 2:]
            data_l = np.array(csv_data_l.values[:, 0:self.custom_agent_num]).astype(np.float32)
            L_list_ori = data_l / 1e4
            E_array = np.sum(L_list_ori, axis=0) * 0.2
            L_array = np.max(L_list_ori, axis=0)
            E_arrays.append(E_array)
            L_MaxArray.append(L_array)

        Earrays = np.array(E_arrays)
        L_MaxArray = np.array((L_MaxArray))

        self.E_max = np.max(Earrays, axis=0)
        ...
        pes_indes = set(range(118)) - set(pes_index)  # 非储能节点

        self.Pes_max[list(pes_indes)] = 0
        self.E_max[list(pes_indes)] = 0  # 0130

        self.L_max_ori = np.max(L_MaxArray, axis=0)
        # self.L_max_ori = np.array([self.L_max] * self.custom_agent_num)
        self.L_max = np.max(self.L_max_ori)
        return

    def init_every_day(self):
        selected_day = np.random.randint(low=0, high=30)
        print("select %d day" % selected_day)
        csv_data_l = self.bus_data_selection['FL'].iloc[selected_day * 24:(selected_day + 1) * 24, 2:]
        csv_data_p = self.bus_data_selection['PV'].iloc[selected_day * 24:(selected_day + 1) * 24, 2:]

        self.L_list_day_array = csv_data_l
        data_l = np.array(csv_data_l.values[:, 0:self.custom_agent_num]).astype(np.float32)
        data_p = np.array(csv_data_p.values[:, 0:self.custom_agent_num]).astype(np.float32)
        self.L_list_ori = data_l / 1e4
        self.P_array_ori = data_p / 1e4
        ...
        # self.E_max = np.sum(self.L_list_ori, axis=0) * 0.2 # 0119
        # print("E_max shape :", self.E_max.shape)
        # self.Pes_max = self.E_max * 0.33 # 0119
        # print("Pes_max shape :", self.Pes_max.shape)
        # 0130 pes_index = self.bus_data_selection['储能节点编号'].values.reshape(-1).tolist()
        # 0130 pes_index = [i - 1 for i in pes_index]
        # 0130 pes_indes = set(range(118)) - set(pes_index)
        # self.Pes_max[list(pes_indes)] = 0
        # self.E_max[list(pes_indes)] = 0
        self.L_sum_i = np.sum(self.L_list_ori, axis=0)

        # self.L_max = np.max(self.L_list_ori) # 0119
        self.L_min = np.min(self.L_list_ori)
        # self.L_max_ori = np.array([self.L_max] * self.custom_agent_num) # 0119
    ...
    def step(self, actions):
        ...
        for i in range(self.agent_num - 1):
            ...
            self.action_state[self.agent_selection][3] = np.clip(self.action_state[self.agent_selection][3],
                                                                 self.action_3_low_limit,
                                                                 self.action_3_high_limit[i])
            ...
            '''
            # 0202
            if (self.num_moves + 1) % 24 == 1:
                # 第一个时刻，已在0130处修改，此处不做处理
                pass
            elif (self.num_moves + 1) % 24 == 23:
                # 倒数第二个时刻
                if ((self.real_state[self.agent_selection][2] - 1.05 * max(0,
                                                                           self.action_state[self.agent_selection][0]))
                        - 0.95 * min(0, self.action_state[self.agent_selection][0]) < 0):
                    self.action_state[self.agent_selection][0] = self.real_state[self.agent_selection][2] / 1.05
                if ((self.real_state[self.agent_selection][2] - 1.05 * max(0,
                                                                           self.action_state[self.agent_selection][0]))
                        - 0.95 * min(0, self.action_state[self.agent_selection][0]) > 0.66 * self.E_max[i]):
                    self.action_state[self.agent_selection][0] = (self.real_state[self.agent_selection][2] - 0.66 *
                                                                  self.E_max[i]) / 0.95
            elif (self.num_moves + 1) % 24 == 0:
                # 倒数第一个时刻
                if self.real_state[self.agent_selection][2] < 0.33 * self.E_max[i]:
                    self.action_state[self.agent_selection][0] = -(
                            self.real_state[self.agent_selection][2] - 0.33 * self.E_max[i]) / 0.95
                if self.real_state[self.agent_selection][2] > 0.33 * self.E_max[i]:
                    self.action_state[self.agent_selection][0] = (self.real_state[self.agent_selection][2]
                                                                  - 0.33 * self.E_max[i]) / 1.05
            else:
                # 其他时刻
                if ((self.real_state[self.agent_selection][2] - 1.05 * max(0,
                                                                           self.action_state[self.agent_selection][0]))
                ...
                if ((self.real_state[self.agent_selection][2] - 1.05 * max(0,
                                                                          self.action_state[self.agent_selection][0])
                        - 0.95 * min(0, self.action_state[self.agent_selection][0])) > self.E_max[i]):
                    self.action_state[self.agent_selection][0] = (self.real_state[self.agent_selection][2] - (
                        self.E_max[i])) / 0.95

        action_ = dc(actions[-1])
        self.agent_selection = self.agents[-1]
        self.action_state[self.agent_selection] = action_
        ...
        info1_lst = dc([i.tolist() for i in self.action_state.values()])

        check4 = []
        # print(info1_lst)
        if type(info1_lst) is list:
            for l in info1_lst:
                if len(l) > 1:
                    check4.append(l[3])

        info2_lst = dc(list(self.tr_dic.values()))
        infos.append(info1_lst)
        infos.append(info2_lst)
        ...
        for index, i in enumerate(self.agents[:-1]):
            ...
            self.real_state[i][2] = self.E_max[index]  # 0130
    ...
    def range_norm(self, bottom_bound, up_bound, value):
        normed_value = (value - (bottom_bound + up_bound) / 2) / ((up_bound - bottom_bound) / 2 + 1e-10)
        return normed_value
