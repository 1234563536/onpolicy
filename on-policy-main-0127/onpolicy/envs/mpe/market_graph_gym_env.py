...
class MA_ELEC_Market_Graph_ENV(gym.Env):
    ...
    def step(self, actions):
        ...
        for i in range(self.agent_num - 1):
            ...
            assert self.action_state[self.agent_selection][3] >=0., "Check Point 1 !"
            assert self.action_3_high_limit[i] > 0., "Check Point 2 !"
            ...
            assert self.action_state[self.agent_selection][3] >= 0., "Check Point 3 !"
            '''
            ...
            assert self.action_3_high_limit[i] > 0., "Check Point 1 !"
            assert self.action_state[self.agent_selection][3] >= 0., "Check Point 2 !"
            '''
        ...
        if type(info1_lst) is list:
            for l in info1_lst:
                if len(l) > 1:
                    check4.append(l[3])
            assert len(check4) > 0, "Check Point 4 !"
            # assert all(map(lambda x: x >= 0, sum(check4,[]))), "Check Point 5 !"
            assert all(map(lambda x: x >= 0, check4)), "Check Point 5 !"

        info2_lst = dc(list(self.tr_dic.values()))
        infos.append(info1_lst)
        infos.append(info2_lst)
        # infos.append([i.tolist() for i in self.action_state.values()])
        # infos.append(list(self.tr_dic.values()))
        # print('infos',infos)
