...
bus = scio.loadmat(r"C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\case118zh\busdata.mat")["busdata"]
...
allbranch = scio.loadmat(r"C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\allbranch.mat")["allbranch"]  # 读取支路数据
params = dict()
params["busNum"] = 118
...
G_Data = scio.loadmat(r"C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\gendata.mat")["gendata"]  # 读取发电机节点数据
...
class MA_ELEC_Market_Graph_ENV(gym.Env):
    ...
    def __init__(self, max_step, custom_agent_num, gate_num = 10):
        ...
        self.gate_map = np.zeros((self.gate_num, 2))
        self.gate_map = np.array([[17,27],[21,22],[27,46],[29,38],[99,77],[71,72],[88,75],[102,103],[108,109],[118,110]])
        self.except_gate = np.array([[8,24],[25,35],[9,40],[43,54],[37,62],[49,62],[58,96],[91,73],[83,108],[86,105]])

        self.custom_matrix = np.random.randint(2, size=(custom_agent_num, custom_agent_num))
        self.custom_agent_num = custom_agent_num
        csv_data_l = pd.read_excel(r'C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\20230201code+data\data_l_118.xlsx')

        self.bus_data_selection=pd.read_excel(r'C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\118bus_data.xlsx',sheet_name=None)
        ...
        csv_data_p = pd.read_excel(r'C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\20230201code+data\data_p_118.xlsx')
        ...
        self.E_max = np.sum(self.L_list_ori,axis=0)*0.2*1.5
        ...
        self.L_max_i = np.sum(self.L_list_ori,axis=0)*1.5
        print('L_max_i',self.L_max_i)


        self.L_max = np.max(self.L_list_ori)*1.5
        self.L_min = np.min(self.L_list_ori)*1.5
        ...
        self.F = scio.loadmat(r"C:\Users\Li Ma\Desktop\on-policy-1122\on-policy-main\onpolicy\envs\mpe\20230201code+data\FL.mat")['FL']
        ...
        self.observation_space.append(Box(low=-1, high=1, shape=(self.gate_num+self.edge_num_max+2,)))
        share_obs_dim = 3*custom_agent_num + self.gate_num+self.edge_num_max+2
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(share_obs_dim,), dtype=np.float32) for _ in range(self.custom_agent_num+1)]
    ...
    def step(self, actions):
        ...
        for i in range(self.agent_num-1):
            ...
            self.s_l_state[self.agent_selection] = self.s_l_state[self.agent_selection] + self.action_state[self.agent_selection][3]
            if self.s_l_state[self.agent_selection] > self.sl_sum[i]:
                # print()
                self.action_state[self.agent_selection][3] = self.action_state[self.agent_selection][3] - self.s_l_state[self.agent_selection] + self.sl_sum[i]
        ...
        # print('elf.action_state',self.action_state[self.agents[7]])
        action = actions[-1]
        ...
        for i in self.agents[:-1]:
            if self.action_state[i][0] > 0:
                ...
                self.tr_dic[i] = self.real_state[i][1] + self.action_state[i][0] \
                              + self.action_state[i][1] - self.real_state[i][1]
        ...
        PN = np.array([(self.P_u[agent]/1e4) - (self.tr_dic[i] /1e4) for agent in self.agents[:-1]])
        ...
        QN = self.L_list_ori[self.num_moves-1,:]
        print('QN',QN)
        ...
        for i in range(self.elec_graph_data.shape[0]):
            if self.elec_graph[int(self.elec_graph_data[i,0])-1, int(self.elec_graph_data[i,1])-1] == 1:
                ...
                for j in range(L.shape[0]):
                    # print('L_0',L[j,0])
                    # print('L_1', L[j, 1])
                    if int(L[j,0]) == (int(self.elec_graph_data[i,0])-1) and int(L[j,1]) == (int(self.elec_graph_data[i,1])-1):
                        l_data = L[j,2]
                        # print('i_reward', l_data)
                        break
                    elif int(L[j,1]) == (int(self.elec_graph_data[i,0])-1) and int(L[j,0]) == (int(self.elec_graph_data[i,1])-1):
                        l_data = L[j, 2]
                        # print('i_reward', l_data)
                        break
    ...
    def get_net_observation(self):
        ...
        obs[self.gate_num:self.custom_agent_num+self.gate_num] = np.zeros((self.custom_agent_num))
        print('num_moves',self.num_moves)
        obs[-2] = self.sell_price_list[self.num_moves-1]
        obs[-1] = 3.5
        return obs
    ...
    def elec_flow(self,p_,q_,r_,x_,custom_agent_num):
        ...
        def nonlinear_constraint(x):
            ...
            p_p = x[:self.custom_agent_num*self.custom_agent_num].reshape((self.custom_agent_num,self.custom_agent_num))
            q_q = x[self.custom_agent_num*self.custom_agent_num:self.custom_agent_num * self.custom_agent_num*2].reshape(self.custom_agent_num, self.custom_agent_num)
            v_ = x[
                ...
                for j in range(self.custom_agent_num):
                    eq_constraint_v[i,j] = v_[i] - 2 * (
                                self.r_[i, j] * p_p[i, j] + self.x_[i, j] * q_q[i, j]) + \
                                                 (self.r_[i, j] * self.r_[i, j] + self.x_[i, j] * self.x_[i, j]) \
                                                 * l_[i, j] - v_[j]
                    neq_constraint_pqv[i,j] = l_[i,j] + v_[i] - np.linalg.norm([2*q_q[i,j], 2*p_p[i,j], l_[i,j]-v_[i]])



            return np.concatenate([eq_constraint_p,eq_constraint_q,eq_constraint_v.reshape(-1),neq_constraint_pqv.reshape(-1)])
        ...
        # 初始猜测
        x0 = np.zeros((self.custom_agent_num * self.custom_agent_num * 2 + self.custom_agent_num + self.custom_agent_num * self.custom_agent_num,))
        # print('x_0',x0.shape)
        # 调用minimize函数
